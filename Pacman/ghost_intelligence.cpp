...
bool isFree(char pos){
    //Check if a given position is free (no obstacles on it)
...
vector<Position> getAllNodes(vector<string> &maze){
    ...
    for (size_t i = 0 ; i < maze.size() ; ++i) {
        for (size_t j = 0 ; j < maze[i].size() ; ++j) {
            if (isFree(maze[i][j])) nodes.push_back(Position{static_cast<unsigned int>(j), static_cast<unsigned int>(i)});
        }
    }
    return nodes;
}
...
//    }
    string direction = aStar(maze, ghost.pos, pacmanPos);
    if (direction != "up" && direction != "down" && direction != "right" && direction != "left") {
        direction = possibleDirections(ghost.pos, maze)[0];
    }
    return aStar(maze, ghost.pos, pacmanPos);
}
